...
const HF_KEY = process.env.HF_KEY || process.env.HF_CREDENTIALS;
const HF_API_KEY = process.env.HF_API_KEY;
const HF_API_SECRET = process.env.HF_API_SECRET;

const HIGGSFIELD_MODEL =
  process.env.HIGGSFIELD_MODEL || "flux-pro/kontext/max/text-to-image";

const HIGGSFIELD_BASE_URL =
  process.env.HIGGSFIELD_BASE_URL || "https://platform.higgsfield.ai";

const HIGGSFIELD_POLL_LIMIT_MS = Number(
  process.env.HIGGSFIELD_POLL_LIMIT_MS || 55000
);
...
function higgsfieldHeaders(): Record<string, string> {
  ...
  if (!credential) return { "Content-Type": "application/json" };
...
function absoluteHiggsfieldUrl(pathOrUrl: string) {
  if (pathOrUrl.startsWith("http://") || pathOrUrl.startsWith("https://")) {
    return pathOrUrl;
  }

  return `${HIGGSFIELD_BASE_URL}${pathOrUrl.startsWith("/") ? "" : "/"}${pathOrUrl}`;
}
...
function extractStatusUrl(data: any): string | null {
  return (
    data?.status_url ||
    data?.response_url ||
    data?.poll_url ||
    data?.job_url ||
    data?.jobs?.[0]?.status_url ||
    data?.jobs?.[0]?.response_url ||
    (data?.request_id ? `/requests/${data.request_id}/status` : null) ||
    (data?.id ? `/requests/${data.id}/status` : null) ||
    null
  );
}

function extractError(data: any): string | null {
  const detail = data?.detail || data?.details || data?.message || data?.error || data?.reason;

  if (typeof detail === "string") return detail;
  if (detail) return JSON.stringify(detail).slice(0, 600);
  return null;
}
...
async function downloadAsBase64(url: string) {
  ...
  if (!res.ok) {
    throw new Error(`Could not download generated image: ${await res.text()}`);
  }
...
async function pollHiggsfieldResult(statusUrl: string) {
  ...
  while (Date.now() - started < HIGGSFIELD_POLL_LIMIT_MS) {
    ...
    if (!statusRes.ok) {
      throw new Error(`Higgsfield status error ${statusRes.status}: ${responseText}`);
    }
    ...
    if (["failed", "error", "nsfw", "canceled", "cancelled"].includes(status)) {
      throw new Error(
        `Higgsfield generation ${status}: ${extractError(json) || JSON.stringify(json).slice(0, 500)}`
      );
    }
  ...
  throw new Error(
    `Higgsfield polling timed out. Last response: ${JSON.stringify(lastJson).slice(0, 700)}`
  );
}

function buildHiggsfieldBody(prompt: string) {
  // Important:
  // Your current endpoint returns: "prompt is a required property".
  // So the prompt must be sent at TOP LEVEL, not inside { input: { prompt } }.
  return {
    prompt,
...
async function generateWithHiggsfield(opts: {
  postId: string;
  prompt: string;
  path?: string;
  ...
  if (!credentials) {
    return {
      imageUrl: null,
      error:
        "Missing Higgsfield credentials. Add HF_KEY=api_key:api_secret or HF_API_KEY + HF_API_SECRET in Vercel.",
    };
  }

  if (!opts.prompt || opts.prompt.trim().length < 10) {
    return {
      imageUrl: null,
  ...
  try {
    ...
    const res = await fetch(endpoint, {
      method: "POST",
      headers: higgsfieldHeaders(),
      body: JSON.stringify(buildHiggsfieldBody(opts.prompt)),
      cache: "no-store",
    });

    const responseText = await res.text();
    const data = parseJsonLoose(responseText);

    if (!res.ok) {
      return {
        imageUrl: null,
        error: `Higgsfield submit error ${res.status}: ${responseText}`,
      };
    }
    ...
    if (!generatedUrl) {
      ...
      if (!statusUrl) {
        return {
          imageUrl: null,
          error: `Higgsfield response missing image/status URL: ${JSON.stringify(data).slice(0, 900)}`,
        };
      }
    ...
    return {
      imageUrl: null,
      error: error?.message || "Unknown Higgsfield image error.",
    };
  }
}
...
async function generateWithOpenAI(opts: {
  ...
  try {
    ...
    if (!res.ok) {
      return {
        imageUrl: null,
        error: `OpenAI image error: ${await res.text()}`,
      };
    }
    ...
    return {
      imageUrl: publicUrl,
      error: publicUrl ? null : "Supabase upload failed.",
    };
  } catch (error: any) {
    return {
      imageUrl: null,
      error: error?.message || "Unknown OpenAI image error.",
    };
  }
}

export async function generateEditorialImage(opts: {
  postId: string;
  prompt: string;
  path?: string;
}): Promise<ImageProviderResult> {
  if (process.env.GENERATE_IMAGES !== "true") {
    return {
      imageUrl: null,
      error: "GENERATE_IMAGES is not set to true.",
    };
  }
  ...
  if (hf.error && process.env.IMAGE_FALLBACK_PROVIDER === "openai") {
    ...
    return {
      ...openai,
      error: openai.error ? `${hf.error} | fallback failed: ${openai.error}` : null,
    };
  }
