...
export default async function StoryPage({ params }: { params: { slug: string } }) {
  ...
  return <main className="page"><div className="shell">
    ...
    <section className="article-grid"><div>
      ...
      <p className="note">Image: Photo illustration, not an actual event photograph.</p>
    ...
    </div><aside className="sidebar"><div className="mini-list"><h3>Reporter</h3><div className="author">{post.author_avatar_url ? <img className="avatar avatar-img" src={post.author_avatar_url} alt={`Editorial portrait for ${post.author_name}`} /> : <span className="avatar">{author.initials}</span>}<span><strong>{post.author_name}</strong><br /><small>{post.author_title}</small>{post.author_photo_note ? <><br /><small className="avatar-note">{post.author_photo_note}</small></> : null}</span></div><p className="note">Beat: {author.beat}</p></div><div className="mini-list"><h3>More in {post.category}</h3>{related.map((item) => <Link href={`/story/${item.slug}`} key={item.id}><div className="section-kicker">{item.subcategory}</div><strong>{item.title}</strong></Link>)}</div></aside></section>
  </div></main>;
}
