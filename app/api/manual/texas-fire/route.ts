...
export async function GET(req: NextRequest) {
  ...
  const prompt = "Create a highly realistic AI-generated editorial image of firefighters responding to a large warehouse fire at night in Texas. Flames and smoke rise from an industrial building, emergency lights reflect on wet pavement, firefighters stand at a safe distance in turnout gear, realistic urban-industrial surroundings, premium American news photojournalism style, dramatic but believable, no logos, no text overlays, no identifiable private people, clearly illustrative and not an actual event photograph.";
  const image = await generateAndUploadImage({ postId: "manual-texas-warehouse-fire-001", prompt });
  ...
  return NextResponse.json({ ok: true, image_error: image.error, post });
}
